          ...
          <div className="app-card p-8 space-y-4">
            <div className="flex items-start justify-between gap-4">
              ...
              <button
                type="button"
                onClick={fetchStats}
                className="btn-press p-2 rounded-lg bg-slate-50 border border-slate-200 hover:bg-slate-100 text-slate-600 transition-colors shrink-0 group"
                title="Refresh results"
              >
                <RefreshCw className="w-4 h-4 icon-spin-hover" />
              </button>
            </div>

            {/* Summary stats */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
              <div className="app-card-interactive bg-slate-50 border border-slate-200 rounded-lg p-4 flex items-center gap-3 group">
                <Users className="w-5 h-5 text-blue-600 shrink-0 icon-float" />
                <div>
            ...
            {stats.totalVotes === 0 ? (
              ...
              <div className="space-y-4">
                {stats.options
                  ...
                  .map((opt, idx) => (
                    <div key={opt.option} className="space-y-1.5 group">
                      <div className="flex justify-between items-center text-xs font-semibold">
                        <div className="flex items-center gap-1.5">
                          {opt.option === leader?.option && stats.totalVotes > 0 && (
                            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
                          )}
                          <span className="text-slate-800 group-hover:text-blue-600 transition-colors">{opt.option}</span>
                        </div>
